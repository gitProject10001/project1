...
import cloudShadowUniformsGlsl from './shaders/cloudShadowUniforms.glsl';
import cloudShadowFunctionsGlsl from './shaders/cloudShadowFunctions.glsl';
...
// Cloud shadow GLSL code — imported from external files, injected into MeshPhongMaterial via onBeforeCompile
...
patchMaterial.onBeforeCompile = (shader) => {
  ...
  shader.fragmentShader = shader.fragmentShader.replace(
    'void main() {',
    cloudShadowUniformsGlsl + '\nvarying vec3 vCloudWorldPos;\n' + cloudShadowFunctionsGlsl + '\nvoid main() {'
  );
